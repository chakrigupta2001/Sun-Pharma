# ====================================================================================
# Script Name   : Disabled_Computers_180Days.ps1
# Description   : Retrieves disabled computer accounts from multiple domains
#                 where 'WhenChanged' is older than 180 days.
...
# Notes         : Structured similar to All Computers script for consistency.
...
$reportName = "Disabled_Computers_180Days_" + $datetime + ".csv"
...
#$domainControllers = @("spil.com","spapl.com","alkaloida.com","spiinc.com","taro.corp","ca.taro.corp","il.taro.corp","us.taro.corp","ranbaxy.com","us.ranbaxy.com","pharmalucence.com","ransap.com")
...
foreach ($domain in $domainControllers) {
    ...
    while ((Get-Date) -lt $startTime.AddMinutes(10) -and -not $connected) {
        try {
            # --- Step 4: Retrieve disabled computers older than 180 days ---
...
if ($totalComputerCount -gt 0) {
    ...
    Write-Output "No Disabled computers found for the past 180 days across all domains | $computerCountsJson | $totalComputerJson"
}