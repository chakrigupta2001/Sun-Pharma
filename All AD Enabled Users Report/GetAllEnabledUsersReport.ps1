...
$outputCsvPath = Join-Path -Path "C:\Temp" -ChildPath $reportName
...
foreach ($domain in $domainControllers) {
    ...
    while ((Get-Date) -lt $startTime.AddMinutes(10) -and -not $connected) {
        try {
            # Step 3: Query all enabled users from the current domain using the selected DC 
            Get-ADUser -Filter {Enabled -eq $true} -Server $dcToUse -Properties `
                ...
                Title, Department, Country, physicalDeliveryOfficeName, ExtensionAttribute8, ExtensionAttribute11 |
